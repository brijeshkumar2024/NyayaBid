...
function resetSessionEnvironment(force = false) {
...
function extractProcurementFields(text) {
  try {
    ...
    const tenderIdPatterns = [
      /(?:Tender\s+Reference\s+No\.?|Tender\s+Ref\.?\s+No\.?|NIT\s+No\.?|Tender\s+No\.?|Reference\s+No\.?)\s*[:\-–—]?\s*([A-Z0-9][A-Z0-9\-\/.() ]{5,}[A-Z0-9\)])/im,
      /\b(?:TND|NIT|RFP|RFQ|GEM)[-\/]?[A-Z0-9][A-Z0-9\-\/]{3,}[A-Z0-9\)]\b/i,
      /(?:Tender\s+Reference|Tender\s+Ref|Reference|Tender)\s*[:\-–—]?\s*([A-Z0-9][A-Z0-9\-\/.() ]{5,}[A-Z0-9\)])/im,
      /\b(?:Tender|Tender\s+Ref|Tender\s+No|Reference\s+No)\b\s*[:\-–—]?\s*([A-Z0-9][A-Z0-9\-\/.() ]{4,})/im
    ];
    let tenderIdMatch = null;
    ...
    if (!fields.tenderId) {
      const tenderHintMatch = normalizedText.match(/(?:Tender\s+Reference\s+No\.?|Tender\s+Ref\.?\s+No\.?|NIT\s+No\.?|Tender\s+No\.?|Reference\s+No\.?)\s*[:\-–—]?\s*([A-Z0-9][A-Z0-9\-\/.() ]{2,})?/im);
      if (tenderHintMatch) {
        fields.tenderIdHint = 'Tender reference ID present in text but exact value could not be confidently extracted';
        addEvidence(evidence, 'tenderIdHint', fields.tenderIdHint, tenderHintMatch, 'tender id hint');
      }
    }
    ...
    if (Number.isFinite(turnoverCr) && turnoverCr > 0 && turnoverMatch) {
      ...
      const turnoverPercentMatch = normalizedText.match(/annual\s+average\s+turnover(?:\s+during\s+the\s+last\s+three\s+years)?\s*.*?\b(?:at\s+least|min(?:imum)?\s+)?(\d{1,3})%\s*of\s*estimated\s*cost\b/i);
      if (turnoverPercentMatch) {
        fields.turnoverRequirementText = `Annual turnover ≥ ${turnoverPercentMatch[1]}% estimated cost`;
        addEvidence(evidence, 'turnoverRequirementText', fields.turnoverRequirementText, turnoverPercentMatch, 'turnover requirement');
      }
    }
    ...
    if (expMatch) {
      ...
      const experiencePhraseMatch = normalizedText.match(/(?:three|4|four|five)\s+similar\s+(?:completed\s+)?works\b/i);
      if (experiencePhraseMatch) {
        fields.experienceRequirementText = 'Prior similar government work experience required';
        addEvidence(evidence, 'experienceRequirementText', fields.experienceRequirementText, experiencePhraseMatch, 'experience requirement');
      }
    }
    ...
    // Solvency / financial capacity
    const solvencyMatch = normalizedText.match(/(?:solvency|financial capacity|bank solvency).{0,140}?\b(?:(?:₹?\s*[\d,]+(?:\.\d+)?\s*(?:lakh|lac|cr|crore))|minimum\s*[\d,]+(?:\.\d+)?)/i);
    if (solvencyMatch) {
      fields.solvencyRequirementText = 'Solvency requirement detected';
      addEvidence(evidence, 'solvencyRequirementText', fields.solvencyRequirementText, solvencyMatch, 'solvency requirement');
    }

    // Contractor registration requirement
    const contractorRegMatch = normalizedText.match(/(?:government|govt\.?|public)\s+registered\s+(?:contractor|contractors|firm|agency)/i);
    if (contractorRegMatch) {
      fields.contractorRegistrationText = 'Government contractor registration mandatory';
      addEvidence(evidence, 'contractorRegistrationText', fields.contractorRegistrationText, contractorRegMatch, 'contractor registration');
    }
